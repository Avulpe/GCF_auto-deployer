 ...
 */
function downloadRepo(user, repoName, filePath, req, res) {
 ...
 * @param {Object} res Cloud Function response context.
 */
function zipDir(filePath, req, res) {
  ...
  zipFolder('/' + filePath, '/tmp/' + archiveName, function(err) {
    if (err) {
      ...
      uploadToGoogleStorage(config.stageBucket, archiveName, req, res);
    }
  });
}
 ...
 * @param {Object} res Cloud Function response context.
 */
function uploadToGoogleStorage(stageBucket, fileToSend, req, res) {
  var gcs = Gcs();
  ...
  bucket.upload('/tmp/' + fileToSend, function(err, file) {
    if (err) {
      ...
      for (var i in config.deployments)
      {
        ...
        if (deployment.path.includes(req.repository.name))
        {
          deployFunction(
              process.env.GCLOUD_PROJECT, stageBucket, deployment.functionName,
              config.location, fileToSend, req, res);
        }
      }
    }
  });
...
function deployFunction(
    projectName, stageBucket, functionName, functionLocation, archiveName, req,
    res) {
...
exports.deployHttp = function deployHttp(req, res) {
  ...
  downloadRepo(
      obj.repository.owner.name, obj.repository.name, filePath, obj, res);
};
